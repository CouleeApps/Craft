...
static Texture SignTexture      = {5, GL_TEXTURE5};
...
void set_block(int x, int y, int z, int w, int b) {
    ...
    for (int dx = -1; dx <= 1; dx++) {
        for (int dz = -1; dz <= 1; dz++) {
            ...
            _set_block(p + dx, q + dz, x, y, z, -w, b);
        }
    }
...
void render_signs(Attrib *attrib, Player *player) {
    ...
    glUseProgram(attrib->program);
    glUniformMatrix4fv(attrib->matrix, 1, GL_FALSE, matrix);
    glUniform1i(attrib->sampler, SignTexture.index);
...
void render_inventory_text(Attrib *attrib, Item item, float x, float y, float n, int scale) {
    float matrix[16];
    // render text
    set_matrix_2d(matrix, width, height);
    ...
    char text_buffer[16];
    float ts = INVENTORY_FONT_SIZE * scale;
    ...
    if (!typing) {
        if (!inventory_screen) {
            ...
            if (key == CRAFT_KEY_ITEM_PREV) {
                inventory.selected = (inventory.selected + INVENTORY_SLOTS + 1) % INVENTORY_SLOTS;
            }
...
int main(int argc, char **argv) {
    ...
    glActiveTexture(SignTexture.glIndex);
